...
class Game:
    ...
    def add_player_locked(self, reconnect_token: str | None = None) -> Player:
        if reconnect_token:
            existing = next(
                (p for p in self.players.values() if p.reconnect_token == reconnect_token),
                None,
            )
            if existing:
        ...
        if self.match_state != "lobby":
            ...
            player.was_active = False
        self.players[player.id] = player
        if self.match_state == "lobby":
            self._update_countdown_locked()
        return player

    async def add_player(self, reconnect_token: str | None = None) -> Player:
        async with self._lock:
    ...
    async def remove_player(self, player_id: str) -> None:
        async with self._lock:
            ...
            if self.match_state == "lobby":
                ...
                if self.countdown_until and not self._all_ready_locked():
                    self.countdown_until = None
            else:
                player.disconnected_at = time.monotonic()
                asyncio.create_task(self._delayed_remove_player(player_id))

    async def _delayed_remove_player(self, player_id: str) -> None:
        await asyncio.sleep(30)
        async with self._lock:
            player = self.players.get(player_id)
            if player and player.disconnected_at is not None:
                self.players.pop(player_id, None)

    def _is_present_locked(self, player: Player) -> bool:
        return not player.spectator and player.disconnected_at is None
    ...
    async def handle_message(self, player_id: str, msg: dict[str, Any]) -> None:
        async with self._lock:
            ...
            if typ == "set_name":
                ...
                self._update_countdown_locked()
            elif typ == "input" and self._is_present_locked(player):
            ...
            elif typ == "jump" and self.match_state == "running" and not player.dead and self._is_present_locked(player):
                if self._is_ice_blocked_locked(player, time.monotonic()):
                    return
                player.jump_until = time.monotonic() + 0.36
            elif typ == "select_target" and self._is_present_locked(player):
            ...
            elif typ == "choose_lobby_upgrade" and self.match_state == "lobby" and self._is_present_locked(player):
    ...
    def _start_wave_locked(self, number: int) -> None:
        ...
        if boss_wave:
            ...
            cycle = (number - 1) // len(regular_waves)
            for typ, count in wave["composition"].items():
                # Repeated wave sets gain bodies as well as stats, so wave 11+ is
                # not just wave 1 with a larger health bar.
                scaled_count = count * (0.45 + 0.18 * scale) * (1 + cycle * 0.22)
                for _ in range(max(1, math.ceil(scaled_count))):
    ...
    def spawn_enemy_locked(self, typ: str, position: dict[str, float] | None = None) -> Enemy:
        ...
        late_waves = max(0, wave_number - 7)
        health_multiplier = 1 + (wave_number - 1) * 0.15 + late_waves * late_waves * 0.012
        damage_multiplier = 1 + (wave_number - 1) * 0.105 + late_waves * 0.012
        elite = not boss and wave_number >= 8 and self._enemy_seq % 5 == 0
        if elite:
            health_multiplier *= 1.3
            damage_multiplier *= 1.2
        speed_multiplier = 1 + min(0.22, max(0, wave_number - 6) * 0.012)
        enemy = Enemy(
            id=f"enemy_{self._enemy_seq}", type=data["id"], name=("Empowered " if elite else "") + data.get("name", data["id"].title()), x=x, z=z,
            hp=round(data["maxHealth"] * health_multiplier, 1), max_health=round(data["maxHealth"] * health_multiplier, 1),
            damage=data["damage"] * damage_multiplier, attack_interval=max(0.72, data["attackInterval"] * (1 - min(0.18, late_waves * 0.008))),
            attack_range=data["attackRange"], move_speed=data["moveSpeed"] * speed_multiplier, armor=data["armor"],
            resistance=data["resistance"], xp=int(data["xp"] * (1 + (wave_number - 1) * 0.05)), boss=boss,
            facing=random.random() * math.tau,
            special_attack_at=time.monotonic() + random.uniform(2.5, 4.0) if boss or wave_number >= 5 else 0,
        )
        self._choose_wander_target_locked(enemy)
    ...
    def _tick_players_locked(self, now: float, dt: float) -> None:
        for player in self.players.values():
            ...
            if self._is_ice_blocked_locked(player, now):
                dx = 0
                dz = 0
                player.input = {}
            ...
            if player.casting:
                self._tick_channel_cast_locked(player, now)
    ...
    def _add_enemy_warning_locked(self, enemy: Enemy, x: float, z: float, effect_type: str, ability_id: str, radius: float, damage: float, school: str, impact_at: float) -> None:
        self._ground_effect_seq += 1
        self.ground_effects.append({
            "id": f"{effect_type}_{self._ground_effect_seq}", "type": effect_type,
            "sourceId": enemy.id, "abilityId": ability_id, "x": x, "z": z,
            "radius": radius, "damage": damage, "school": school,
            "impactAt": impact_at, "expiresAt": impact_at + 0.25,
        })
        self._emit_locked({"type": "status", "sourceId": enemy.id, "targetId": enemy.id, "abilityId": ability_id, "status": "warning", "duration": max(0, impact_at - time.monotonic())})

    def _enemy_target_locked(self, enemy: Enemy, now: float) -> Player | None:
        if now < enemy.stun_until:
            return None
        living = [p for p in self.players.values() if not p.dead and self._is_present_locked(p) and not self._is_stealthed_locked(p, now)]
        if not living:
            return None
    ...
    def _cast_ability_locked(self, player: Player, slot: int, ground_position: dict[str, Any] | None = None) -> None:
        ...
        if cast_time > 0:
            ...
            for effect in ability.get("effects", []):
                if effect.get("type") == "channel_damage":
                    player.casting["channelNextTick"] = now
                    player.casting["channelTickInterval"] = effect.get("tickInterval", 0.5)
                    break
    ...
    def _charge_locked(self, player: Player, enemy: Enemy, effect: dict[str, Any], ability_id: str) -> None:
        ...
        stop = effect.get("stopDistance", 1.45)
        player.x = enemy.x - dx / dist * stop
        player.z = enemy.z - dz / dist * stop
        self._push_out_of_map_objects_locked(player)
        arena = self.constants["arenaRadius"] - 1
        current = math.hypot(player.x, player.z)
        if current > arena:
            player.x *= arena / current
            player.z *= arena / current
        player.facing = math.atan2(enemy.x - player.x, enemy.z - player.z)
        stun = effect.get("stunDuration", 0)
        if stun:
            now = time.monotonic()
    ...
    def _tick_ground_effects_locked(self, now: float) -> None:
        for effect in list(self.ground_effects):
            if now >= effect["expiresAt"] and effect.get("type") not in ("boss_meteor", "enemy_meteor", "enemy_charge"):
            ...
            if effect.get("type") == "trap":
                ...
                for enemy in list(self.enemies.values()):
                    ...
                    if effect in self.ground_effects:
                        self.ground_effects.remove(effect)
                    break
            elif effect.get("type") in ("boss_meteor", "enemy_meteor", "enemy_charge"):
                if now < effect.get("impactAt", now):
                    continue
                source = self.enemies.get(effect.get("sourceId", ""))
                if effect.get("type") == "enemy_charge" and source:
                    source.x, source.z = effect["x"], effect["z"]
                    self._push_out_of_map_objects_locked(source)
    ...
    def _tick_stat_buffs_locked(self, player: Player, now: float) -> None:
        for buff in list(player.stat_buffs):
            ...
            if "stat" not in buff:
                ...
                continue
            stat = buff["stat"]
            player.stats[stat] = buff.get("previous", player.stats.get(stat, 0))
            player.stat_buffs.remove(buff)

    def _tick_channel_cast_locked(self, player: Player, now: float) -> None:
        casting = player.casting
        if not casting or now < casting.get("channelNextTick", now + 1):
            return
        ability_id = casting.get("abilityId")
        ability = self.abilities.get(ability_id)
        if not ability:
            return
        target = self.enemies.get(casting.get("targetId") or "")
        if not target or self._distance(player, target) > ability.get("range", 0) or self._line_of_sight_blocked_locked(player.x, player.z, target.x, target.z):
            return
        for effect in ability.get("effects", []):
            if effect.get("type") != "channel_damage":
                continue
            amount = effect.get("amount", 0) + player.stats.get(effect.get("scaling", {}).get("stat", ""), 0) * effect.get("scaling", {}).get("coefficient", 0)
            self._damage_enemy_locked(player, target, amount, effect.get("school", "arcane"), ability.get("threatMultiplier", 1), ability_id)
            break
        casting["channelNextTick"] = now + casting.get("channelTickInterval", 0.5)
    ...
    def _damage_player_locked(player: Player, damage: float) -> float:
        if Game._is_ice_blocked_locked(player, time.monotonic()):
            return 0
    ...
    @staticmethod
    def _is_ice_blocked_locked(player: Player, now: float) -> bool:
        return any(buff.get("abilityId") == "mage_ice_block" and now < buff.get("endAt", 0) for buff in player.stat_buffs)
    ...
    def _cycle_target_locked(self, player: Player, ally: bool) -> None:
        if ally:
            ids = [p.id for p in self.players.values() if p.id != player.id and self._is_present_locked(p)]
    ...
    def _compute_match_stats_locked(self) -> dict[str, dict[str, Any]]:
        ...
        for pid, p in self.players.items():
            stats[pid] = {
                "name": p.name,
                "classId": p.class_id,
                "spectator": p.spectator or p.disconnected_at is not None,
                "level": p.level,
    ...
    def _snapshot_locked(self, player_id: str | None = None, events: list[dict[str, Any]] | None = None, include_static: bool = True) -> dict[str, Any]:
        ...
        if player_id and player_id in self.players:
            snapshot["reconnectToken"] = self.players[player_id].reconnect_token
        return snapshot
