...
def test_every_class_offers_one_additional_iconic_spell_in_coop():
    game = Game()
    expected = {
    ...
    assert set(expected) == set(game.classes)
    for class_id, ability_id in expected.items():
        ...
        player = Player(id=class_id, name=class_id, class_id=class_id)
        player.abilities = list(game.classes[class_id]["startingAbilities"])
        choices = {choice["id"] for choice in game._level_choices_locked(player)}
        assert f"learn:{ability_id}" in choices


def test_mage_blink_moves_forward_without_reusing_disengage_semantics():
    game = Game()
    game.map_objects.clear()
    mage = Player(id="mage", name="Mage", class_id="mage")
    mage.stats = dict(game.classes["mage"]["baseStats"])
...
def test_mage_can_learn_every_spell_with_a_unique_action_slot():
    ...
    assert len(mage.abilities) == 12
    assert len(set(assigned_slots)) == 12
    assert max(assigned_slots) == 12
