...
export function renderPvpLobby(state:any,send:(message:Record<string,unknown>)=>void){
  ...
  lobby.querySelector("#pvpHint")!.textContent=state.countdown!==null?`Starting in ${Math.ceil(state.countdown)}…`:!me.team?"Choose a team":!me.classId?"Choose a class":me.build.length!==state.buildPoints?`${me.build.length}/${state.buildPoints} talents assigned`:me.ready?"Waiting for opponent…":"Ready to fight";lobby.querySelector("#pvpCountdown")!.textContent=state.countdown!==null?`Battle begins in ${Math.ceil(state.countdown)}`:"";
