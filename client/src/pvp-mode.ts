...
export function adaptPvpSnapshot(raw:any,previous:any){
  ...
  const players=Object.fromEntries(Object.entries<any>(raw.players||{}).map(([id,p])=>{const serverAbilities=Array.isArray(p.abilities)?p.abilities:[];const learnedAbilities=serverAbilities.length?serverAbilities:(Array.isArray(p.build)?p.build.filter((choice:string)=>choice.startsWith("spell:")).map((choice:string)=>choice.slice(6)):[]);const serverSlots=Object.fromEntries(Object.entries(p.abilitySlots||{}).map(([abilityId,slot])=>[abilityId,Number(slot)]));const abilitySlots=learnedAbilities.every((abilityId:string)=>Number.isFinite(serverSlots[abilityId]))?serverSlots:Object.fromEntries(learnedAbilities.map((abilityId:string,index:number)=>[abilityId,index+1]));const activeEffects=(p.activeEffects||[]).map((effect:any,index:number)=>({...effect,id:effect.id||`${id}:${effect.sourceId||"unknown"}:${effect.abilityId||"effect"}:${effect.kind||"status"}:${index}`,sourceId:effect.sourceId||"",abilityId:effect.abilityId||"unknown_effect",permanent:Boolean(effect.permanent),stacks:Number(effect.stacks||1)}));return[id,{...p,activeEffects,abilities:learnedAbilities,abilitySlots,level:1,xp:0,jumping:Boolean(p.jumping),jumpProgress:p.jumpProgress||0,pendingUpgrades:[],autoAttack:{remaining:0,interval:p.stats?.autoAttackInterval||1.5,progress:0},baseStats:classes[p.classId]?.baseStats||p.stats||{}}]}));
